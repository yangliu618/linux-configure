...
(function(window) {
    ...
    var APFRequire = function(urls, type, callback) {
        ...
        var self = this;
        window.basket.isValidItem = self.isValidItem;
        window.basket.require.apply(window.basket, this.urls).then(function() {
            self.resolve.call(self, self.urls);
        }, function(e) {
            self.reject.call(self);
        });
    }
    ...
    APFRequire.prototype = {
        ...
        resolve : function(data) {
            ...
            for(k in data) {
                ...
                (function(key) {
                    var rs = window.basket.get(key);
                    result.push(rs);
                    self.type === 'js' ? self.createTextScript(rs) : self.createTextStyle(rs);
    ...
    window.APFLoad = (function() {
        return {
            require : function(urls, type, callback) {
                new APFRequire(urls, type, callback);
            },
            clear : function(fun) {
